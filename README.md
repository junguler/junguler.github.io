...
the reason for making an empty file is github automatically removes empty directories/folders inside your repo, you can remove the empty txt file as long as you have uploaded one other file inside that folder which can be accessed by clicking `add file` and choosing `upload files`
